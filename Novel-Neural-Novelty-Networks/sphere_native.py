#!/usr/bin/env python3
"""
Sphere Native - Directional Weight Encoding on Fibonacci Lattice
...
class SphereNativeLayer:
    ...
    def _reconstruct_weight_scaled(self):
        """
        ...
        mag_fp = mag_fp * self.row_mag_scale[:, np.newaxis]  # (out, n_groups)
    ...
    def forward_fp32(self, x_fp32):
        """
        Forward pass.
        y_j = Σ_k dot(x_triplet_k, mag_jk * TABLE[idx_jk])
        """
        ...
        w_scaled = self._reconstruct_weight_scaled()

        out = np.einsum('snd,ond->so', x_trips, w_scaled)
    ...
    def forward_fp32_fast(self, x_fp32):
        """Alias — same implementation, the 'fast' path is the only path now."""
        return self.forward_fp32(x_fp32)
...
class SphereNativeModel:
    ...
    def __init__(self, config: Dict[str, Any], gpu: bool = False):
        ...
        for lc in config.get('layers', []):
            ...
            self.layer_names.append(lc.get('name', f'layer_{len(self.layers)-1}'))
...
class SphereNative:
    ...
    def forward_full(self, token_ids):
        ...
        for layer_idx in range(n_layers):
            ...
            # --- QKV projections (sphere encoded) ---
            ...
            q = q_layer.forward_fp32_fast(x_normed)
            k = k_layer.forward_fp32_fast(x_normed)
            v = v_layer.forward_fp32_fast(x_normed)
            ...
            # O projection
            attn_out = o_layer.forward_fp32_fast(attn_out)
            ...
            if use_swiglu:
                ...
                gate_out = gate_layer.forward_fp32_fast(x_normed2)
                up_out = up_layer.forward_fp32_fast(x_normed2)

                silu_gate = gate_out * (np.float32(1.0) / (np.float32(1.0) + np.exp(-gate_out)))
                mlp_out = silu_gate * up_out

                mlp_out = down_layer.forward_fp32_fast(mlp_out)
            else:
                up_layer = self._get_layer(f'layer.{layer_idx}.mlp.up')
                down_layer = self._get_layer(f'layer.{layer_idx}.mlp.down')

                mlp_out = up_layer.forward_fp32_fast(x_normed2)
    ...
    def stream(self, prompt, max_tokens=100, temperature=0.7, top_k=50, stop_token=None):
        """Stream tokens. Yields one token string at a time."""
        ...
        for _ in range(max_tokens):
            hidden = self.forward_full(input_ids)
            ...
            yield self.tokenizer.decode_token(next_id)
